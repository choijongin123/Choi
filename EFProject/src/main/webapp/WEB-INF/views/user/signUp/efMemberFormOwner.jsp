...
<head>
	 ...
	 <script src="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.2/js/bootstrap.min.js"></script>
			...
			function fnc_process() { // 아이디 중복 검사
		        var tcn = document.getElementById('ef_c_name');
		        var tid = document.getElementById('ef_id');
		        var tts = "0";
		        var RegExpCn = /^[가-힣-a-zA-Z0-9]{1,20}$/;
		        var id_RegExpCn = /^[a-zA-Z0-9]{4,12}$/;
			    var	_cName	= $("#ef_c_name").val(), _id1	= $("#ef_id").val();
			        
			   
				        $.ajax({  // 업체명 중복 체크 기능
							type:		"post",
							url:		"${contextPath}/user/checkCname.do",
							dataType:	"json",
							async : false,
							data:		{ef_c_name: _cName},	// 업체명을 서블릿으로 전송한다.
								success:	function(data) {
									
								// 서버에서 전송된 결과에 따라 메시지를 표시한다.

									if (tcn.value == '' || tcn.value == null){      		// 업체명 입력란이 공백인 경우.
										$("#cnChkMsg").html("업체명을 입력해주세요.");
										$("#cnChkMsg").css("color","white")
										tts = "0";
									} else if (!RegExpCn.test(tcn.value)){					// 업체명에 특수문자나 공백이 들어간 경우.
										$("#cnChkMsg").html("특수문자는 사용할 수 없습니다.");
										$("#cnChkMsg").css("color","pink")
							            tts = "0";
							        } else if ($.trim(data) == 0) {							// 업체명이 DB와 겹치지 않는 경우.
										$("#cnChkMsg").html("등록 가능한 업체명 입니다.");	
										$("#cnChkMsg").css("color","skyblue")
										tts = "1";
									} else {												// 업체명이 DB와 겹치는 경우.
										$("#cnChkMsg").html("이미 등록된 업체명 입니다.");	
										$("#cnChkMsg").css("color","pink")	
										tts = "0";
									}
										$.ajax({	// 아이디 중복 체크 기능
												...
												// 서버에서 전송된 결과에 따라 메시지를 표시한다.
													...
													if (tid.value == '' || tid.value == null){				// 아이디 입력란이 공백인 경우.
														...
														$("#subBtn").prop("disabled", true);
											        } else if ($.trim(data) == 0 && tts==1) {				// 아이디가 DB와 겹치지 않고 업체명이 DB와 겹치지 않는 경우.
														$("#idChkMsg").html("사용 가능한 아이디 입니다.");	
														$("#idChkMsg").css("color","skyblue")
														$("#subBtn").prop("disabled", false);
													} else if ($.trim(data) == 0 && tts==0) {				// 아이디가 DB와 겹치지 않고 업체명이 DB와 겹치는 경우.
														$("#idChkMsg").html("사용 가능한 아이디 입니다.");	
														$("#idChkMsg").css("color","skyblue")
														$("#subBtn").prop("disabled", true);
				...
				<form method="post" name="memInsForm">
					...
					<table id="joinTable">
						<tr style="display:none;">
							<th><input type="text" id="ef_class" name="ef_class" value="01"></th> 
						</tr>		
						<tr> <!--  업체명 등록 관련  -->				
							<td><span class="joinText">업체명</span></td>
							<td><input type="text" id="ef_c_name" class="joinInput" name="ef_c_name" maxlength="20" placeholder="업체명 입력" oninput="fnc_process()"/></td>		
						</tr>
						<tr>		
							<td><span class="joinText2">√ 업체명 중복 체크 : </span></td>
							<td id="cnChkMsg">업체명을 입력해주세요.</td>
						</tr>		
						<tr>
							<td><span class="joinText">아이디</span></td>
							<td><input type="text" id="ef_id" class="joinInput" name="ef_id" maxlength="12" placeholder="아이디 입력" oninput="fnc_process();"></td>
						<tr>
						...
						<tr>
							...
							<td><input type="password" id="ef_pwd" class="joinInput" name="ef_pwd" maxlength="12" placeholder="비밀번호 입력"/></td>
						</tr>
						
						<tr>
							<td><span class="joinText">비밀번호 확인</span></td>
							<td><input type="password" id="repwd" class="joinInput" name="repwd" maxlength="12" placeholder="비밀번호 다시 입력"/></td>
						</tr>
						<tr>
							<td><span class="joinText">이 름</span></td>
							<td><input type="text" id="ef_name" class="joinInput" name="ef_name" maxlength="12" placeholder="이름 입력"/></td>
						</tr>		
						<tr>
							<td><span class="joinText">전화번호</span></td>
							<td><input type="text" id="ef_p_number" class="joinInput" name="ef_p_number" maxlength="12" placeholder="전화번호 입력"/></td>
						</tr>		
						<tr>
							<td><span class="joinText">이메일</span></td>
							<td><input type="text" id="ef_email" class="joinInput" name="ef_email" maxlength="50" placeholder="이메일 입력"/></td>
						</tr>		
						<tr>
